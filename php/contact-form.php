...
$email = 'hello@albasonamakeup.com';

// If the e-mail is not working, change the debug option to 2 | $debug = 2;
$debug = 0;

// If contact form don't has the subject input change the value of subject here
$subject = (isset($_POST['subject'])) ? $_POST['subject'] : 'Contacto de ' . $_POST['name'] . ' desde albasonamakeup.com';

$message = '';

foreach($_POST as $label => $value) {
	// Use the code below to change label texts. On this example will change "Email" to "Email Address"
	...
	if( $label == 'place' ) {               
		$label = 'Estudio/desplazamiento';              
	}

	if( $label == 'address' && empty($value) ) {
        continue;
    }
